...
public class MainChat extends JFrame implements ActionListener, Runnable{
   ...
   public void connect(){//(소켓)서버연결 요청
	   try {
		  ...
		  Socket s = new Socket("localhost", 8000);//연결시도		  
		   ...
		   while(true){
			   ...
			   switch(protocol){
			   	...
			   	case "202"://개설된 방의 타이틀 제목 얻기
			   		...
			   		break;
			   	case "800"://아디디변경
			   		cc.ta.append("=========["+msgs[1]+"]님으로 이름 변경=========\n");
			   		cc.ta.setCaretPosition(cc.ta.getText().length());
			   		break;
			   	case "500"://비밀채팅
			   		cc.ta.append(msgs[2] + "님에게 귓속말이 전달되었습니다.\n");
			   		cc.ta.setCaretPosition(cc.ta.getText().length());
			   		break;
			   		}//클라이언트 switch
			   }
